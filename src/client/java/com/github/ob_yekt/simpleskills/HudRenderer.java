...
public class HudRenderer {
    ...
    private static String generateSkillText() {
        ...
        // Check if classes are enabled in the configuration
        if (ConfigLoader.isFeatureEnabled("classes")) {
            // Fetch the player's class
            String playerClass;
            try {
                playerClass = db.getPlayerClass(playerUUID);
            } catch (Exception e) {
                playerClass = "Unknown"; // Fallback if unable to fetch
                System.out.println("[simpleskills] Failed to fetch player class: " + e.getMessage());
            }

            // Add player class at the top, styled
            hudText.append("§f⚔ Class: ").append(playerClass != null ? playerClass : "Peasant").append(" ⚔\n");
        } else {
        ...
        try (var rs = db.getPlayerSkills(playerUUID)) {
            while (rs.next()) {
                ...
                skills.put(skillName, new SkillData(skillName, currentLevel, currentXP));
            }
        } catch (Exception e) {
